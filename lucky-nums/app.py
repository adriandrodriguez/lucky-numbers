...
@app.route('/api/get-lucky-num', method=['POST'])
def lucky_num_data():
    error = {'error': {}}
    ...
    if 'email' not in request.json or request.json['email'] is '': error['error']['email'] = 'The email field is required'   
    if 'year' not in request.json or request.json['year'] is '': error['error']['year'] = 'The year field is required'
    ...
    if 'color' not in request.json or request.json['color'] is '':
        ...
        error['error']['color'] = 'Invalid value, must be one of: red, green, blue, orange'
